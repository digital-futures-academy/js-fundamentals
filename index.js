...
if (job === "enc") {
  return fs.readFile(`${__dirname}/${fileName}`, "UTF-8").then((plaintext) => {
    if (cipherType === "ll") {
    ...
    if (cipherType === "ln") {
      ...
      return letterNumber.encryptAsync(plaintext, key).then((encodedText) => {
        console.log(encodedText);
        fs.writeFile(`${__dirname}/${fileName}.enc`, encodedText, "UTF-8").then(
          () => {
  ...
  return fs.readFile(`${__dirname}/${fileName}`, "UTF-8").then((plaintext) => {
    if (cipherType === "ll") {
      ...
      return letterLetter.decryptAsync(plaintext).then((decodedText) => {
        ...
        fs.writeFile(
          `${__dirname}/${fileName.substring(0, fileName.indexOf(".enc"))}`,
          decodedText,
          "UTF-8"
        )
          .then(() => {
            console.log(
              `decoded file ${fileName.substring(
                0,
                fileName.indexOf(".enc")
              )} created`
            );
            return;
          })
    ...
    if (cipherType === "ln") {
      ...
      return letterNumber.decryptAsync(plaintext, key).then((decodedText) => {
        console.log(decodedText);
        fs.writeFile(
          `${__dirname}/${fileName.substring(0, fileName.indexOf(".enc"))}.enc`,
          decodedText,
          "UTF-8"
        ).then(() => {
          console.log(
            `encoded file ${fileName.substring(
              0,
              fileName.indexOf(".enc")
            )} created, key: ${key}`
          );
