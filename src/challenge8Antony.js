...
class LetterNumber {
    ...
    decrypt(string, offset) {
        if(typeof string !== 'string' || typeof offset !== 'number') {console.log('Please enter valid inputs'); return false}
        else {
            ...
            for(let i = 0; i < string.length - 1; i += 2) {         //  perform action on every other index
                let twoDigits = parseInt(string.slice(i, i + 2));   //  slice the original string into 2 digit pieces
                while(twoDigits < offset) {
                    twoDigits += 99;
                }
                twoDigits -= offset;                                //  Convert input string digits into the actual key value pair numbers
                twoDigitArray.push(twoDigits.toString());           //  Add these digits to an
            }
